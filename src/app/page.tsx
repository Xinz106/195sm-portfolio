...
export default function App() {
  ...
  // 动画状态机：-2=加载中, -1=黑场, 0=渲染计算, 1=中心显现, 2=全貌展现
  const [animStage, setAnimStage] = useState(-2);

  useEffect(() => {
    // 💡 模拟加载过程，确保浏览器有充足时间完成布局预热
    ...
    return () => { 
      clearTimeout(t_load); clearTimeout(t0); 
  ...
  const socials = [
    {
      name: "Instagram",
      url: "https://www.instagram.com/xinz106",
      icon: (
    ...
    {
      name: "Twitter",
      url: "https://x.com/195_sm",
      icon: (
        <svg className="w-4 h-4 md:w-5 md:h-5 shrink-0 fill-current" viewBox="0 0 512 512">
    ...
    {
      name: t.contact,
      url: "mailto:909158176a@gmail.com",
      icon: (
        <svg className="w-4 h-4 md:w-5 md:h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <main className="relative w-full min-h-screen bg-[#050505] overflow-x-hidden text-gray-200 font-sans selection:bg-gray-700 selection:text-white pb-20">
      ...
      {/* 3. 物理 Banner 层 */}
      <motion.div
        className="absolute top-0 left-0 w-full h-[480px] md:h-[600px] z-[0] overflow-hidden bg-black"
        initial={{ opacity: 0 }}
        animate={{ opacity: animStage === 2 ? 1 : 0 }}
      ...
      <AnimatePresence>
        {animStage === -1 && (
          <motion.div className="fixed inset-0 bg-[#050505] z-[100]" exit={{ opacity: 0 }} transition={{ duration: 0.4 }} />
        )}
      </AnimatePresence>
      ...
      {animStage === 1 && (
        <div className="fixed inset-0 flex items-center justify-center pointer-events-none z-[50]">
          <motion.h1
            ...
            transition={{ layout: springTransition, opacity: { duration: 0.6 } }}
            className="text-6xl md:text-8xl font-black text-white tracking-widest drop-shadow-[0_0_25px_rgba(255,255,255,0.5)]"
          >
            195sm
          </motion.h1>
        </div>
      ...
      <div className="max-w-[1400px] mx-auto pt-24 px-6 md:px-12 relative z-10">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-6 md:gap-10">
          ...
          <motion.div
            className="hidden md:flex w-48 h-48 rounded-full border border-gray-800 bg-gray-900 shrink-0 shadow-[0_0_30px_rgba(0,0,0,0.8)] overflow-hidden items-center justify-center relative"
            initial={{ opacity: 0, x: -20 }}
            animate={{ opacity: animStage === 2 ? 1 : 0, x: animStage === 2 ? 0 : -20 }}
          ...
          <div className="flex-1 flex flex-col items-start w-full">
            ...
            <motion.div
              ...
              animate={{ opacity: animStage === 2 ? 1 : 0, y: animStage === 2 ? 0 : 10 }}
              transition={{ duration: 0.8, delay: 0.3 }}
              className="space-y-3"
            >
              <h2 className="text-xl md:text-2xl font-bold tracking-widest text-gray-400">
                {t.role}
              </h2>
          ...
          <motion.div
            className="hidden md:flex flex-col items-end w-auto pt-4 md:pt-1.5"
            initial={{ opacity: 0, x: 20 }}
            animate={{ opacity: animStage === 2 ? 1 : 0, x: animStage === 2 ? 0 : 20 }}
        ...
        {/* 物理分界横线 */}
        <motion.div 
           className="w-full h-px bg-gray-800 my-16 md:my-24 relative"
           initial={{ scaleX: 0, opacity: 0 }}
           animate={{ scaleX: animStage === 2 ? 1 : 0, opacity: animStage === 2 ? 1 : 0 }}
           transition={{ duration: 1.2, delay: 0.6 }}
        >
          <div className="absolute inset-0 shadow-[0_12px_40px_rgba(0,0,0,1)]" />
        </motion.div>

        {/* 💡 修复重点：作品集区域移除背景、模糊和圆角，实现完全的“合为一体” */}
        <motion.div 
          className="w-full space-y-20 md:space-y-32 pb-32 relative z-[20] px-2"
          initial={{ opacity: 0, y: 40 }}
          animate={{ opacity: animStage === 2 ? 1 : 0, y: animStage === 2 ? 0 : 40 }}
          transition={{ duration: 1, delay: 0.8 }}
        >
          {/* 3D 作品集 */}
          <div className="block group">
            <h3 className="text-xl md:text-2xl font-bold mb-10 md:mb-16 text-white text-center group-hover:drop-shadow-[0_0_15px_rgba(255,255,255,0.4)] transition-all tracking-[0.6em]">
              {t.p3d}
            </h3>
            <a href="/portfolio-3d" className="relative block w-full aspect-[4/3] md:aspect-[21/9] bg-[#0a0a0a] border border-white/5 rounded-2xl overflow-hidden shadow-2xl group-hover:border-white/20 transition-all duration-700">
              <img 
                src="/portfolio-3d.jpg" 
                alt="3D Portfolio" 
                className="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-100 group-hover:scale-[1.04] transition-all duration-1000" 
          ...
          {/* 代码作品集 */}
          <div className="block">
            <h3 className="text-xl md:text-2xl font-bold mb-10 md:mb-16 text-gray-500 text-center tracking-[0.6em]">
              {t.pCode}
            </h3>
            <div className="w-full aspect-[4/3] md:aspect-[21/9] bg-[#070707] border border-white/10 rounded-2xl overflow-hidden relative shadow-2xl">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <p className="text-lg md:text-3xl font-bold tracking-[1em] text-white/10">
                  {t.comingSoon}
                </p>
              </div>
            </div>
      ...
      <div className="absolute top-10 right-12 z-[60]">
        <button 
          ...
          className="flex items-center gap-2 px-4 py-2 bg-black/50 border border-white/10 backdrop-blur-md rounded-md transition-all text-sm font-medium tracking-wider text-white"
        >
          {langMap[lang]}
        ...
        <AnimatePresence>
          {isLangOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="absolute top-full right-0 mt-2 w-full min-w-[100px] bg-[#0a0a0a]/90 border border-gray-700 rounded-md backdrop-blur-xl shadow-2xl flex flex-col overflow-hidden"
            >
              {(Object.entries(langMap) as [keyof typeof langMap, string][]).map(([key, label]) => (
                <button
                  key={key}
                  onClick={() => { setLang(key); setIsLangOpen(false); }}
                  className={`px-4 py-3 text-sm transition-colors hover:bg-white/10 ${lang === key ? "text-white font-bold bg-white/5" : "text-gray-400"}`}
                >
                  {label}
                </button>
              ))}
